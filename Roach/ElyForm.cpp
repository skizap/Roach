...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case EUI_EXECUTE:
		{
			...
			luaL_dostring(c_L, code);
			delete[] code;
			break;
		}
		case EUI_CREDITS:
			MessageBox(form->GetWindow(), "X-Tools", "Credits", MB_OK);
			break;
		case EUI_EXIT:
...
HMENU Form::create_window_menu() {
	...
	AppendMenu(_menu, MF_POPUP, (UINT_PTR)main_dropdown, "Roach");
...
int Form::watch_directory(Form* form) {
	char fPath[MAX_PATH];
	HMODULE nigge2r;
	util::GetFile("Roach.dll", "scripts", fPath, MAX_PATH);
